...
public:
  GoalMarkerPublisher()
      : Node("goal_marker_publisher"), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_)
  {
    // ---------------- Parameters ----------------
    ...
    this->declare_parameter<std::string>("agnostic_goal_topic",
                                         "/debug/agnostic_goal"); // PoseStamped for G0
    ...
    conf_sub_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
        conf_topic_, 1, [this](const std_msgs::msg::Float64MultiArray::SharedPtr msg) {
    ...
    soft_goal_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        soft_goal_topic_, 1, [this](const geometry_msgs::msg::PoseStamped::SharedPtr p) {
          last_soft_goal_ =
              Eigen::Vector3d(p->pose.position.x, p->pose.position.y, p->pose.position.z);
          last_soft_goal_quat_ = Eigen::Quaterniond(p->pose.orientation.w, p->pose.orientation.x,
    ...
    agnostic_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        agnostic_goal_topic_, 1, [this](const geometry_msgs::msg::PoseStamped::SharedPtr p) {
          last_g0_pos_ =
              Eigen::Vector3d(p->pose.position.x, p->pose.position.y, p->pose.position.z);
        });

    raw_velocity_sub_ = this->create_subscription<geometry_msgs::msg::Vector3>(
        raw_velocity_topic_, 1, [this](const geometry_msgs::msg::Vector3::SharedPtr v) {
          last_vE_ = Eigen::Vector3d(v->x, v->y, v->z);
          have_vE_ = true;
        });

    dynamic_goal_sub_ = this->create_subscription<extender_msgs::msg::SharedControlGoalArray>(
        "/shared_control/dynamic_goals", 10,
        std::bind(&GoalMarkerPublisher::dynamicGoalCallback, this, std::placeholders::_1));
...
private:
  ...
  std::map<int, Goal> active_goals_;
  ...
  // Subscription for goal
  rclcpp::Subscription<extender_msgs::msg::SharedControlGoalArray>::SharedPtr dynamic_goal_sub_;
  rclcpp::TimerBase::SharedPtr timer_;
  tf2_ros::Buffer tf_buffer_;
  tf2_ros::TransformListener tf_listener_;

  void dynamicGoalCallback(const extender_msgs::msg::SharedControlGoalArray::SharedPtr msg)
  {
    active_goals_.clear();
    for (const auto &goal_msg : msg->goal_array)
    {
      Goal dg(goal_msg.goal_pose.position.x, goal_msg.goal_pose.position.y,
                             goal_msg.goal_pose.position.z);
      active_goals_.insert({goal_msg.id, dg});
    }
  }
  // ------------------------------------------------------------
  void onTimer()
  {
    ...
    if (!have_tf)
    {
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                           "TF lookup failed (%s -> %s). Check frame names.", base_frame_.c_str(),
                           ee_frame_.c_str());
    ...
    Eigen::Vector3d ee_pos(T.transform.translation.x, T.transform.translation.y,
  ...
  void publishMarkers(std::optional<Eigen::Vector3d> ee_pos_opt)
  {
    ...
    if (last_g0_pos_.has_value())
    {
      ...
      arr.markers.push_back(
          basicText(now, base_frame_, 10000, agn + Eigen::Vector3d(0, 0, 0.07), "(agnostic)"));
      ...
      if (draw_vectors_ && have_vE_)
      {
        ...
        arr.markers.push_back(
            basicArrow(now, base_frame_, 50000, agn, tip, 0.004, 0.01, 1.0, 0.0, 0.0, 0.9));
      }
    }
    else
    {
    ...
    for (size_t i = 0; i < goals_.size(); ++i)
    {
      ...
      // main goal sphere
      auto m = basicSphere(now, base_frame_, static_cast<int>(i + 1), p, is_max ? 0.08 : 0.05);
      m.ns = "goals";
      if (is_max)
      {
        m.color.r = 1.0;
      ...
      auto txt = basicText(now, base_frame_, static_cast<int>(2000 + i),
                           p + Eigen::Vector3d(0, 0, 0.07), "G" + std::to_string(i + 1));
      ...
      if (ee_pos_opt.has_value())
      {
        ...
        auto cone =
            makeCone(now, base_frame_, static_cast<int>(3000 + i), ee, p, theta_l_rad_, dist);
    ...
    {
      const Eigen::Vector3d p(goal.x, goal.y, goal.z);
      const bool is_max = (id == max_goal_local_idx);

      // main goal sphere
      auto m = basicSphere(now, base_frame_, static_cast<int>(id + 1), p, is_max ? 0.08 : 0.05);
      m.ns = "goals";
      if (is_max)
      {
        m.color.r = 1.0;
      ...
      // r1 translucent sphere
      auto r1m = basicSphere(now, base_frame_, static_cast<int>(1000 + id), p, 2.0 * r1_);
      r1m.ns = "r1_spheres";
      r1m.color.r = 0.8;
      r1m.color.g = 0.8;
      r1m.color.b = 0.0;
      ...
      // r2 translucent sphere (assistance gate σ_d=0 inside r2)
      auto r2m = basicSphere(now, base_frame_, static_cast<int>(11000 + id), p, 2.0 * r2_);
      r2m.ns = "r2_spheres";
      r2m.color.r = 0.0;
      r2m.color.g = 0.4;
      r2m.color.b = 1.0;
      ...
      // label "G<i>"
      auto txt = basicText(now, base_frame_, static_cast<int>(2000 + id),
                           p + Eigen::Vector3d(0, 0, 0.07), "G" + std::to_string(id + 1));
      arr.markers.push_back(txt);
      ...
      if (ee_pos_opt.has_value())
      {
        ...
        double dist = (p - ee).norm();
        auto cone =
            makeCone(now, base_frame_, static_cast<int>(3000 + id), ee, p, theta_l_rad_, dist);
    ...
    {
      const auto &ee = *ee_pos_opt;
      Eigen::Vector3d g(goals_[max_goal_local_idx].x, goals_[max_goal_local_idx].y,
                        goals_[max_goal_local_idx].z);
      ...
      arr.markers.push_back(
          basicArrow(now, base_frame_, 60000, ee, tip, 0.004, 0.01, 0.0, 1.0, 1.0, 0.9));
    }
    ...
    if (last_soft_goal_.has_value())
    {
      ...
      arr.markers.push_back(
          basicText(now, base_frame_, 70001, sg + Eigen::Vector3d(0, 0, 0.07), "Soft"));
    }
    ...
    {
      addAxes(now, base_frame_, 80000, *last_soft_goal_, *last_soft_goal_quat_, axes_len_,
              axes_shaft_d_, axes_head_d_, arr);
    }
  ...
  // Helpers
  // ------------------------------------------------------------
  void addAxes(const rclcpp::Time &stamp, const std::string &frame, int id_base,
               const Eigen::Vector3d &p, const Eigen::Quaterniond &q, double len, double shaft_d,
               double head_d, visualization_msgs::msg::MarkerArray &arr) const
  {
  ...
  visualization_msgs::msg::Marker basicSphere(const rclcpp::Time &stamp, const std::string &frame,
                                              int id, const Eigen::Vector3d &p,
  ...
  visualization_msgs::msg::Marker basicText(const rclcpp::Time &stamp, const std::string &frame,
                                            int id, const Eigen::Vector3d &p,
  ...
  visualization_msgs::msg::Marker basicArrow(const rclcpp::Time &stamp, const std::string &frame,
                                             int id, const Eigen::Vector3d &from,
                                             const Eigen::Vector3d &to, double shaft_diam,
                                             double head_diam, double r, double g, double b,
                                             double a) const
  {
    visualization_msgs::msg::Marker m;
    m.header.frame_id = frame;
  ...
  visualization_msgs::msg::Marker makeCone(const rclcpp::Time &stamp, const std::string &frame,
                                           int id, const Eigen::Vector3d &apex,
                                           const Eigen::Vector3d &goal, double theta,
                                           double length) const
  {
    visualization_msgs::msg::Marker m;
    m.header.frame_id = frame;
